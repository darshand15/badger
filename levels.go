...
func (s *levelsController) addLevel0PartitionedTable(pid int, t *table.Table) error {
	...
	fmt.Println("Inside add Level 0 part table")
	fmt.Printf("Len of L0 Part table: %d\n", len(l0.partitionedTables[pid]))

	fmt.Printf("Key count in table: %d\n", l0.partitionedTables[pid][0].KeyCount())

	// seekKey := y.KeyWithTs([]byte("k1"), uint64(1))
	// logicalKey := y.ParseKey(seekKey)

	for _, tbl := range l0.partitionedTables[pid] {
		...
		// Use a for loop with Rewind, Valid, and Next
		// This will visit every key-value pair in the table
		for itr.Rewind(); itr.Valid(); itr.Next() {
			// itr.Key() returns the full internal key (key + timestamp)
			fullKey := itr.Key()

			// Use y.ParseKey to get just the user's key
			userKey := y.ParseKey(fullKey)

			fmt.Printf("User Key in tbl: %s\n", userKey)
		}
		...
		// k := itr.Key()
		// tbl_userkey := k
		// if len(tbl_userkey) > 8 {
		// 	tbl_userkey = tbl_userkey[:len(tbl_userkey)-8]
		// }
...
func (s *levelsController) get(key []byte, maxVs y.ValueStruct, startLevel int) (
	...
	for _, h := range s.levels {
		...
		// } else {
		// 	// tbls = h.tables
		// 	tbls = h.partitionedTables[0] // dd_modify
		// }
...
func (lc *levelsController) flushEntriesToPartitions(entries []*Entry) error {
	...
	// 	// fmt.Println("print inside flush Entries to partitions")
	...
	// 	userKey := t.Biggest()
	// 	if len(userKey) > 8 {
	// 		userKey = userKey[:len(userKey)-8]
	// 	}
	// 	// Now we can safely print the parsed userKey with %s.
	// 	fmt.Printf("Biggest Key: %-15s\n", userKey)

	// 	smallKey := t.Smallest()
	// 	if len(smallKey) > 8 {
	// 		smallKey = smallKey[:len(smallKey)-8]
	// 	}
	// 	// Now we can safely print the parsed smallKey with %s.
	// 	fmt.Printf("Smallest Key: %-15s\n", smallKey)

	// 	// fmt.Printf("print end of flush Entries to partitions %d", fanout)
	...
	for _, e := range entries {
		...
		// fmt.Printf("Entry Key: %15s\n", e.Key[:len(e.Key)-8])
	}
	fmt.Printf("Len of Parts: %d\n", len(parts[0]))
	for pid, ents := range parts {
		if len(ents) == 0 {
			continue
		}

		fmt.Println("Inside parts range for loop")
...
func (lc *levelsController) buildSSTable(entries []*Entry) (*table.Table, error) {
	...
	for _, e := range entries {
		...
		// fmt.Printf("Inside build SS table for loop for entries\n")
	}
